...
- **[2026-05-07]** | Reschedule cassé : déplacer un meeting libérait l'ancienne salle Skedda + retirait le doc DB, mais ne créait pas le nouveau booking. Cause : quand le user déplace, Google Calendar garde la `location` de l'ancien booking dans l'event. `shouldBookRoom()` voit `location_already_set` et retourne `shouldBook: false`. Le branch `dateChanged` du webhook libérait correctement l'ancienne, puis "tombait" dans le flow de création… qui était gardé par un `if (!decision.shouldBook) continue;` plus bas. Donc release mais pas re-book. | **Quand un branch métier sait déjà que l'opération doit avoir lieu, ne pas le faire passer par les check génériques en aval** : extraire l'opération en helper et l'appeler explicitement (`await startBookingFlow(user, event); continue;`). Aussi : se méfier des fall-through implicites quand des conditions amont vont contredire la décision aval.
- **[2026-05-07]** | Premier vrai booking d'un autre sales (Guillaume) : Skedda a bien créé la résa, on a écrit `location = "Mars"` dans le Calendar event, mais Google a immédiatement re-poussé un webhook pour ce changement. Notre code a vu `location_already_set` sur un meeting qu'il avait lui-même booké et a interprété ça comme "le user a setté la location manuellement" → cancelled la résa Skedda 4s après l'avoir créée. Symptôme côté user : "Mars" affiché dans Google Calendar mais salle libre côté Skedda. | **Quand un service tiers déclenche un webhook après notre propre écriture, prévoir un test d'idempotence** : comparer le state actuel à celui qu'on a posé. Ici, comparer `event.location` à `existing.room` permet de distinguer "écho de notre write" (skip) vs "modification utilisateur" (release). Plus généralement : avant de réagir à un webhook, vérifier qu'on n'est pas soi-même à l'origine du changement.
